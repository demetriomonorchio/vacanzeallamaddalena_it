...
const metaByLocale: Record<Locale, { title: string; description: string }> = {
  it: {
    title: "Tramonto responsabile | Vacanze alla Maddalena",
    description:
      "Guida per vivere il tramonto nell'arcipelago in modo responsabile, evitando vetro abbandonato e comportamenti a rischio.",
  },
  en: {
    title: "Responsible sunset | La Maddalena Holidays",
    description:
      "Guide to enjoy sunset in the archipelago responsibly, avoiding abandoned glass and risky behavior.",
  },
};

function getContent(locale: Locale): string | null {
  const file = path.join(process.cwd(), "content", locale, "tramonto-responsabile.md");
  if (fs.existsSync(file)) return fs.readFileSync(file, "utf8");
  const fallback = path.join(
    process.cwd(),
    "content",
    defaultLocale,
    "tramonto-responsabile.md"
  );
...
export default async function TramontoResponsabilePage({ params }: Props) {
  const { lang } = await params;
